...
- (void)buildEntries
{
    ...
    NSArray* plugins = [ALPHAPluginManager sharedManager].plugins;
    ...
    for (ALPHAPlugin* plugin in plugins)
    {
        ...
        {
            if ([action isKindOfClass:[ALPHAMenuItem class]] && action.isEnabled)
            {
...
{
    ALPHAMenuActionItem* menuAction = self.entries[indexPath.row];
